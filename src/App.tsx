...
function Layout({ children }: { children: React.ReactNode }) {
  ...
  return <>
    ...
    <header className="navbar">
      <Link to="/" aria-label="Rzeszowska Beczka — strona główna"><img src="/06_header_horizontal_transparent_1600x450.png" alt="Rzeszowska Beczka" /></Link>
...
function Footer() {
  return <footer><div className="footer-grid container">
    <div><img className="footer-logo" src="/06_header_horizontal_transparent_1600x450.png" alt="Rzeszowska Beczka" /><p>Whisky bez zadęcia.<br />Smak, historie i dobre rozmowy.</p></div>
    ...
    <div><h3>Bądź blisko</h3><div className="socials"><a href="#instagram" aria-label="Instagram"><Instagram /></a><a href="#facebook" aria-label="Facebook"><Facebook /></a></div><a href="mailto:hello@rzeszowskabeczka.pl">hello@rzeszowskabeczka.pl</a></div>
...
function Home() {
  return <>
    ...
    <section className="flight-promo container"><div className="promo-card"><Eyebrow>Nie możesz się zdecydować?</Eyebrow><h2>Wybierz <em>tasting flight.</em></h2><p>Trzy lub cztery starannie dobrane whisky, podane w kolejności, która ma znaczenie. Mała podróż — bez wychodzenia z baru.</p><Link className="button" to="/degustacje">Zobacz zestawy <ArrowRight /></Link></div><img src="/03_emblem_circle_transparent_1024.png" alt="" /></section>
...
function WhiskyCard({ whisky: w, number }: { whisky: typeof whiskies[number], number: string }) {
  return <Link className="whisky-card" to={`/menu/${w.slug}`}><span className="card-number">{number}</span><p>{w.country} · {w.region}</p><h3>{w.name}</h3><div className="tags">{w.notes.map(n => <span key={n}>{n}</span>)}</div><div className="card-bottom"><span>{w.style}</span><strong>{w.price} zł <small>/ 40 ml</small></strong></div></Link>
}
...
function WhiskyDetail() {
  ...
  return <section className="detail container"><Link className="back" to="/menu">← Wróć do menu</Link><div className="detail-grid"><div className="bottle-shape"><img src="/04_emblem_gold_transparent_1024.png" alt="" /></div><div><Eyebrow>{w.country} · {w.region}</Eyebrow><h1>{w.name}</h1><p className="lead">{w.description}</p><div className="specs"><div><small>STYL</small><strong>{w.style}</strong></div><div><small>WIEK</small><strong>{w.age}</strong></div><div><small>MOC</small><strong>{w.abv}</strong></div></div><h3>Profil smakowy</h3><div className="tags large">{w.notes.map(n => <span key={n}>{n}</span>)}</div><div className="detail-price"><strong>{w.price} zł</strong><span>porcja 40 ml</span></div><Link className="button" to="/rezerwacje">Spróbuj przy barze <ArrowRight /></Link></div></div></section>
}
